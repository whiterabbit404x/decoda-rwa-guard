...
test.describe('asset monitoring column status', () => {
  test('maps backend monitoring_link_status to correct labels', () => {
    // "attached" without explicit telemetry → fail-closed: "Waiting for telemetry" (never optimistic "Monitoring")
    expect(monitoringLinkStatusLabel({ monitoring_link_status: 'attached' })).toBe('Waiting for telemetry');
