...
test('partial endpoint failure behavior is represented via monitoring view model provenance', () => {
  ...
  expect(threat).toContain("type MonitoringProvenanceLabel = 'live' | 'degraded' | 'stale snapshot' | 'partial endpoint failure';");
  expect(threat).toContain("? 'partial endpoint failure'");
...
test('stale-but-visible data behavior is shown from the single view model', () => {
  const threat = source();
  expect(threat).toContain("? 'stale snapshot'");
