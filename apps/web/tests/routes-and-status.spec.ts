...
test('keeps the route split between marketing / and authenticated /dashboard', async () => {
  ...
  // Marketing `/` composes the public company homepage sections and drives
  // conversion through the demo request; it must not pull in the authenticated
  // dashboard data path.
  expect(marketingPage).toContain('HeroSection');
  expect(marketingPage).toContain('RWASecurityPlatformSection');
  expect(marketingPage).toContain('CompanyCTA');
...
test('defines authenticated navigation for dashboard, feature routes, history, and settings', async () => {
  ...
  expect(APP_NAV_ITEMS.map((item) => item.label)).toEqual(['Onboarding', 'Dashboard', 'Assets', 'Monitoring Sources', 'Threat Monitoring', 'Alerts', 'Incidents', 'Response Actions', 'Evidence & Audit', 'Integrations', 'Settings', 'System Health']);
});
