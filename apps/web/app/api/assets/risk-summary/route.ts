...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const response = await fetchWithTimeout(
      `${backendApiUrl}/assets/risk-summary`,
    ...
    if (error instanceof FetchTimeoutError) {
      return jsonError(504, { detail: 'Timed out waiting for backend risk summary.', code: 'backend_timeout', transport: 'same-origin proxy' });
    }
