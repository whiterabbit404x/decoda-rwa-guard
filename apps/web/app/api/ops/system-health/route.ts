...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetchWithTimeout(
      `${backendApiUrl}/ops/system-health`,
      { method: 'GET', headers: buildForwardHeaders(request), cache: 'no-store', next: { revalidate: 0 } },
      PROXY_TIMEOUT_MS,
    );
    const payload = await response.json().catch(() => ({}));
    return Response.json(payload, {
      status: response.status,
      headers: { 'Cache-Control': 'no-store', Vary: 'Authorization, X-Workspace-Id, X-CSRF-Token, Cookie' },
    });
  } catch (error) {
    if (error instanceof FetchTimeoutError) {
      return jsonError(504, { detail: 'Timed out waiting for system health.', code: 'backend_timeout' });
    }
