...
export async function POST(request: Request) {
  ...
  if (!runtimeConfig.configured || !backendApiUrl) {
    return jsonError(500, { detail: 'Runtime proxy is not configured.', code: 'invalid_runtime_config', transport: 'same-origin proxy' });
  }
  ...
  try {
    const response = await fetchWithTimeout(`${backendApiUrl}/monitoring/systems/repair/treasury-settlement-target`, {
      method: 'POST',
      headers,
      cache: 'no-store',
    }, PROXY_TIMEOUT_MS);
    ...
    if (error instanceof FetchTimeoutError) {
      return jsonError(504, { detail: 'Timed out waiting for backend repair response.', code: 'backend_timeout', transport: 'same-origin proxy' });
    }
