...
export async function GET(request: Request) {
  ...
  const backendUrl = `${backendApiUrl}/stream/alerts`;
  ...
  try {
    ...
    return Response.json(
      { detail: 'Alert stream backend unreachable.', code: 'backend_unreachable' },
      { status: 502 },
    );
  }

  if (!backendResponse.ok || !backendResponse.body) {
    const body = await backendResponse.json().catch(() => ({ detail: 'Alert stream unavailable.' })) as Record<string, unknown>;
