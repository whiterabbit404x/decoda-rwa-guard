...
// Proxy for the backend real-time telemetry SSE stream (GET /stream/telemetry).
// Mirrors app/api/stream/alerts/route.ts so the Target Telemetry page can receive
// live wallet-transfer rows over the same authenticated, workspace-scoped, Redis
// backed transport. Streaming responses must never be cached.
...
export async function GET(request: Request) {
  ...
  const backendUrl = `${backendApiUrl}/stream/telemetry`;
  ...
  try {
    ...
    return Response.json(
      { detail: 'Telemetry stream backend unreachable.', code: 'backend_unreachable' },
      { status: 502 },
    );
  }

  if (!backendResponse.ok || !backendResponse.body) {
    const body = await backendResponse.json().catch(() => ({ detail: 'Telemetry stream unavailable.' })) as Record<string, unknown>;
