...
export async function GET(request: Request) {
  ...
  const backendUrl = `${backendApiUrl}/stream/sources`;
  ...
  try {
    ...
    return Response.json(
      { detail: 'Source stream backend unreachable.', code: 'backend_unreachable' },
      { status: 502 },
    );
  }

  if (!backendResponse.ok || !backendResponse.body) {
    const body = await backendResponse.json().catch(() => ({ detail: 'Source stream unavailable.' })) as Record<string, unknown>;
