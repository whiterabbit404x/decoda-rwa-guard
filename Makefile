...
validate-readiness-proof:
	@bash -euo pipefail -c '\
		...
		python -c "import json; s=json.load(open('$$ARTIFACT_DIR/summary.json')); print('controlled_pilot_ready:', s.get('controlled_pilot_ready')); print('broad_self_serve_ready:', s.get('broad_self_serve_ready')); print('enterprise_procurement_ready:', s.get('enterprise_procurement_ready'))"; \
