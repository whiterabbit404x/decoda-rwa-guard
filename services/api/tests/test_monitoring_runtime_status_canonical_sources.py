...
class _Conn:
    def execute(self, q, p=None):
        ...
        if 'FROM target_coverage_records' in qn:
            return _Result(rows=[{'target_id': 't1', 'coverage_status': 'reporting', 'last_telemetry_at': now - timedelta(seconds=10), 'evidence_source': 'live', 'computed_at': now}])
        if 'FROM provider_health_records' in qn:
            return _Result(rows=[{'provider_type': 'rpc', 'target_id': 't1', 'status': 'healthy', 'checked_at': now, 'latency_ms': 12, 'error_message': None, 'evidence_source': 'live', 'metadata': {}}])
        if 'COUNT(*)' in qn:
...
def test_runtime_status_includes_persisted_provider_health_and_target_coverage(monkeypatch):
    monkeypatch.setattr(monitoring_runner, 'resolve_workspace_context', lambda request=None: _Ctx())
    ...
    assert payload['target_coverage']
