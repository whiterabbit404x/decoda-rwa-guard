...
class _Conn:
    ...
    def execute(self, query, params=None):
        ...
        if q.startswith('SELECT id, is_enabled, runtime_status FROM monitored_systems'):
            return _Result(row={'id': self.system['id'], 'is_enabled': self.system['is_enabled'], 'runtime_status': self.system['runtime_status']})
        ...
        if q.startswith('SELECT id, workspace_id, asset_id, target_id, chain, is_enabled, runtime_status, status, last_heartbeat, last_error_text, created_at FROM monitored_systems'):
...
def test_runtime_error_update_does_not_disable(monkeypatch):
    ...
    conn.system['runtime_status'] = 'active'
    ...
    updated = pilot.patch_monitored_system('sys-1', {'runtime_status': 'error'}, _fake_request())
    assert updated['system']['runtime_status'] == 'error'
