...
def run_monitoring_cycle(*, worker_name: str = 'monitoring-worker', limit: int = 50, trigger_type: str = 'scheduler') -> dict[str, Any]:
    ...
    with pg_connection() as connection:
        ...
        if not due_target_ids and skipped_not_due > 0:
            ...
            logger.info(
                'monitoring_due_selection_backfill workspace_target_id=%s reason=all_targets_within_interval oldest_last_checked_at=%s oldest_age_seconds=%s backfill_threshold_seconds=%s should_backfill=%s',
                fallback_target_id or None,
                oldest_checked_at.isoformat() if oldest_checked_at else None,
                oldest_age_seconds,
                max_effective_monitoring_interval_seconds,
                should_backfill,
            )
        logger.info(
