...
CONTINUITY_STATUS_VALUES = {'continuous_live', 'continuous_no_evidence', 'degraded', 'offline', 'idle_no_telemetry'}
...
def evaluate_workspace_monitoring_continuity(
    *,
    ...
    return {
        ...
        'detection_pipeline_age_seconds': detection_age_seconds,
        ...
        'continuity_thresholds_seconds': required_thresholds_seconds,
        'continuity_signals': {
...
def reconcile_workspace_monitored_systems(request: Request) -> dict[str, Any]:
    ...
    with pg_connection() as connection:
        ...
        _append_reconcile_event(
            ...
            reason_code=None,
            detail='Reconcile execution started.',
            payload={'stage': stage},
        )
        ...
        job_counts = {
            ...
            'repaired_monitored_systems': len(result.get('repaired_monitored_system_ids') or []),
            'invalid_targets': reason_counts['invalid'],
            'skipped_targets': reason_counts['skipped'],
        }
        repaired_target_ids = sorted({str(item.get('id')) for item in eligible_targets if item.get('id')})
        ...
        _append_reconcile_event(
            ...
            event_status=run_status,
            reason_code=status_reason_code,
            detail=status_reason_detail or 'Reconcile completed successfully.',
            payload={'reason_counts': reason_counts, 'unresolved_reasons': unresolved_reasons},
        )
        response: dict[str, Any] = {
            'workspace': workspace_context['workspace'],
            'job': _reconcile_job_payload(
                run_id=reconcile_run_id,
                status=run_status,
                counts=job_counts,
                reason_codes=reason_codes,
...
def get_latest_workspace_reconcile_run(request: Request) -> dict[str, Any]:
    require_live_mode()
    with pg_connection() as connection:
        ensure_pilot_schema(connection)
        ...
        if not item:
            return {'workspace': workspace_context['workspace'], 'job': None}
        return {
            'workspace': workspace_context['workspace'],
...
def get_workspace_reconcile_status(request: Request, reconcile_id: str) -> dict[str, Any]:
    require_live_mode()
    with pg_connection() as connection:
        ensure_pilot_schema(connection)
        _user, workspace_context = _require_workspace_admin(connection, request)
...
def get_latest_workspace_reconcile_result(request: Request) -> dict[str, Any]:
    require_live_mode()
    with pg_connection() as connection:
        ensure_pilot_schema(connection)
        _user, workspace_context = _require_workspace_admin(connection, request)
    ...
    *,
    ...
    if identifier:
        ...
        if len(chain_agnostic) == 1:
            ...
            logger.info(
                'orphan_target_relinked target_id=%s workspace_id=%s new_asset_id=%s reason=identifier_match_chain_agnostic',
                target_id, workspace_id, new_asset_id,
            )
            return {'status': 'relinked', 'asset_id': new_asset_id}
        if len(chain_agnostic) > 1:
            return {
                'status': 'multiple_candidates',
                'candidates': [{'id': str(m['id']), 'name': str(m.get('name') or '')} for m in chain_agnostic],
            }
        # No identifier match at all — create an asset from target data
    ...
    if len(ws_assets) == 1:
        ...
        logger.info(
            'orphan_target_relinked target_id=%s workspace_id=%s new_asset_id=%s reason=single_workspace_asset',
            target_id, workspace_id, new_asset_id,
        )
        return {'status': 'relinked', 'asset_id': new_asset_id}
    if len(ws_assets) > 1:
        return {
            'status': 'multiple_candidates',
            'candidates': [{'id': str(a['id']), 'name': str(a.get('name') or '')} for a in ws_assets],
        }
...
def repair_orphan_target(target_id: str, request: Request) -> dict[str, Any]:
    """Attempt to repair an orphaned monitoring target by relinking to a matching workspace asset."""
    require_live_mode()
    with pg_connection() as connection:
        ensure_pilot_schema(connection)
        user, workspace_context = _require_workspace_admin(connection, request)
        workspace_id = workspace_context['workspace_id']
        row = connection.execute(
            'SELECT id, asset_id FROM targets WHERE id = %s AND workspace_id = %s AND deleted_at IS NULL',
            (target_id, workspace_id),
        ).fetchone()
        if row is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Target not found.')
        # Check if already has a valid asset link
        asset_valid = connection.execute(
            'SELECT a.id FROM assets a WHERE a.id = %s::uuid AND a.workspace_id = %s::uuid AND a.deleted_at IS NULL',
            (row.get('asset_id'), workspace_id),
        ).fetchone()
        if asset_valid is not None:
            ...
            return {
                ...
                'monitored_system_id': result.get('monitored_system_id'),
            }
        repair_result = _try_relink_orphan_target(
            connection,
            target_id=target_id,
        ...
        if repair_status not in ('relinked', 'created'):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail='Cannot repair target: no matching asset found and no identifier available to create one. Add a protected asset and link it to this target.',
            )
        ms_result = ensure_monitored_system_for_target(connection, target_id=target_id, workspace_id=workspace_id)
        log_audit(
            connection,
            action='target.repair',
            entity_type='target',
            entity_id=target_id,
            request=request,
            user_id=user['id'],
            workspace_id=workspace_id,
            metadata={'repair_status': repair_status, 'asset_id': repair_result.get('asset_id')},
        )
        connection.commit()
        return {
            'status': repair_status,
            'target_id': target_id,
            'asset_id': repair_result.get('asset_id'),
            'monitored_system_id': ms_result.get('monitored_system_id'),
            'targets_relinked': 1 if repair_status == 'relinked' else 0,
            'assets_created': 1 if repair_status == 'created' else 0,
            'systems_created': 1 if ms_result.get('status') == 'ok' else 0,
        }
...
def execute_enforcement_action(action_id: str, request: Request) -> dict[str, Any]:
    ...
    with pg_connection() as connection:
        ...
        if mode == 'recommended':
            ...
            error_code = 'RESPONSE_ACTION_MANUAL_REQUIRED'
            result_code = 409
            ...
            error_reason = result_summary
            error_code = 'RESPONSE_ACTION_UNSUPPORTED_EXECUTOR'
            result_code = 409
        ...
        metadata['execution_status'] = next_status
        ...
        metadata['tx_hash'] = execution_tx_hash
        ...
        artifacts = _append_status_transition(
            artifacts,
            from_status=str(action.get('status') or ''),
...
def run_guided_threat_workflow(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    with pg_connection() as connection:
        ...
        telemetry_event_record = connection.execute(
            ...
            (telemetry_insert_id, workspace_id, asset['asset']['id'], target['target']['id'], observed_at, observed_at, evidence_source, hashlib.sha256(telemetry_insert_id.encode('utf-8')).hexdigest(), _json_dumps({'stage': 'first_telemetry_ingestion', 'guided_mode': guided_mode, 'runtime_mode': runtime_mode, 'evidence_source': evidence_source, 'has_live_provenance': has_live_provenance, 'evidence_label': evidence_label}), monitoring_run_id),
        ).fetchone()
        ...
        detection_record = connection.execute(
            "INSERT INTO detections (id, workspace_id, monitored_system_id, protected_asset_id, detection_type, severity, confidence, title, evidence_summary, evidence_source, source_rule, status, detected_at, raw_evidence_json, monitoring_run_id, linked_alert_id, created_at, updated_at) VALUES (%s, %s, %s::uuid, %s::uuid, 'guided_monitoring_chain', 'high', 0.95, %s, %s, %s, 'guided.workflow.rule', 'open', %s, %s::jsonb, %s::uuid, NULL, NOW(), NOW()) RETURNING id",
            (detection_insert_id, workspace_id, target['target']['id'], asset['asset']['id'], 'Guided workflow detection', f'Rule evaluation and detection creation from first telemetry ingestion ({evidence_label or evidence_source}).', evidence_source, observed_at, _json_dumps({'telemetry_event_id': telemetry_event_id, 'guided_mode': guided_mode, 'runtime_mode': runtime_mode, 'evidence_source': evidence_source, 'has_live_provenance': has_live_provenance, 'generated_chain_data': generated_chain}), monitoring_run_id),
        ).fetchone()
        detection_id = str((detection_record or {}).get('id') or '')
        alert_insert_id = str(uuid.uuid4())
        alert_record = connection.execute(
            "INSERT INTO alerts (id, workspace_id, user_id, analysis_run_id, alert_type, title, severity, status, source_service, summary, payload, created_at, detection_id, detection_event_workspace_id, detection_event_id, target_id, module_key, source, dedupe_signature, occurrence_count, first_seen_at, last_seen_at, updated_at) VALUES (%s, %s, %s, NULL, 'guided_monitoring_alert', 'Guided workflow alert', 'high', 'open', 'guided-workflow', %s, %s::jsonb, NOW(), %s::uuid, %s, NULL, %s::uuid, 'monitoring', %s, %s, 1, %s, %s, NOW()) RETURNING id",
            (alert_insert_id, workspace_id, user['id'], f'Alert created from guided workflow detection ({evidence_label or evidence_source}).', _json_dumps({'detection_id': detection_id, 'telemetry_event_id': telemetry_event_id, 'guided_mode': guided_mode, 'runtime_mode': runtime_mode, 'evidence_source': evidence_source, 'has_live_provenance': has_live_provenance, 'generated_chain_data': generated_chain}), detection_id, workspace_id, target['target']['id'], evidence_source, f'guided:{detection_id}:{evidence_source}', observed_at, observed_at),
        ).fetchone()
...
def _export_guided_workflow_live_evidence_artifacts(*, workspace_id: str, chain_ids: dict[str, str]) -> None:
    ...
    datasets['report.md'] = '\n'.join(
        [
            ...
            f"- alert: `{bool(chain_ids['alert_id'])}`",
            f"- incident: `{bool(chain_ids['incident_id'])}`",
            f"- response action: `{bool(chain_ids['response_action_id'])}`",
            f"- evidence package: `{bool(chain_ids['evidence_package_id'])}`",
            '',
