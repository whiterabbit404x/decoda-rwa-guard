...
CREATE INDEX IF NOT EXISTS idx_asset_registry_workspace_status
    ON asset_registry (workspace_id, status, created_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS idx_asset_registry_workspace_type_identifier_chain
    ON asset_registry (workspace_id, type, lower(address_or_identifier), lower(chain));
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_monitored_targets_workspace_provider_identifier
    ON monitored_targets (workspace_id, provider_type, target_identifier);
...
CREATE INDEX IF NOT EXISTS idx_detection_events_workspace_created
...
ALTER TABLE incidents
    ADD COLUMN IF NOT EXISTS source_alert_workspace_id UUID NULL;

UPDATE incidents i
SET source_alert_workspace_id = a.workspace_id
FROM alerts a
WHERE i.source_alert_id = a.id
  AND i.source_alert_workspace_id IS NULL;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'incidents_source_alert_workspace_id_fkey'
          AND conrelid = 'incidents'::regclass
    ) THEN
        ALTER TABLE incidents
            ADD CONSTRAINT incidents_source_alert_workspace_id_fkey
            FOREIGN KEY (source_alert_workspace_id, source_alert_id)
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'governance_actions_action_mode_check'
          AND conrelid = 'governance_actions'::regclass
    ) THEN
        ALTER TABLE governance_actions
            ADD CONSTRAINT governance_actions_action_mode_check
            CHECK (action_mode IN ('manual', 'automated', 'simulation'));
    END IF;
END $$;
