...
def build_live_provider_validation(
    ...
    - Reads from live-evidence-proof artifact first (from generate_live_evidence_proof.py),
      falls back to launch-proof artifact for backward compatibility.
    ...
    chain_id_configured = (
        _env_present('CHAIN_ID')
        or _env_present('EVM_CHAIN_ID')
        or _env_present('STAGING_EVM_CHAIN_ID')
    )

    if not evm_rpc_configured:
        blockers.append('EVM_RPC_URL (or STAGING_EVM_RPC_URL) not configured')
    if not chain_id_configured:
        warnings.append('EVM_CHAIN_ID (or STAGING_EVM_CHAIN_ID) not configured')

    # Load live-evidence-proof artifact first (preferred: generated by generate_live_evidence_proof.py)
    if live_evidence_proof_path is None and launch_proof_path is None:
        live_evidence_proof_path = (
            REPO_ROOT / 'artifacts' / 'live-evidence-proof' / 'latest' / 'summary.json'
        )
    live_evidence_proof = (
        _load_json_artifact(live_evidence_proof_path) if live_evidence_proof_path is not None else None
    )

    # Fall back to launch-proof artifact for backward compatibility
    ...
    chain_response_action_id: str | None = None
    chain_evidence_package_id: str | None = None
    contradiction_flags: list[str] = []

    # Prefer canonical live-evidence-proof whenever it is present.
    # This lets staging keep truthful provider readiness even when full
    ...
    def _svc_summary_live() -> dict | None:
        ...
        if not svc_path.exists():
            return None
        try:
            with open(svc_path) as _f:
                _d = json.load(_f)
            if (
                str(_d.get('evidence_source') or '').strip().lower() == 'live'
                and _d.get('live_evidence_ready') is True
                and _d.get('provider_ready') is True
            ):
                return _d
        except Exception:
            pass
    ...
    if _live_ev_proof_active:
        # Use live-evidence-proof artifact (from generate_live_evidence_proof.py)
        lpe = live_evidence_proof.get('live_provider_evidence', {})  # type: ignore[union-attr]
        provider_health_checked = bool(lpe.get('provider_health_checked'))
        provider_mode = str(lpe.get('provider_mode') or 'disabled')
        chain_id_observed = lpe.get('chain_id_observed')
        worker_enabled = bool(lpe.get('worker_enabled'))
        live_evidence_ready = bool(lpe.get('live_evidence_ready'))
        latest_live_telemetry_at = lpe.get('latest_live_telemetry_at')
        evidence_source = str(lpe.get('evidence_source') or 'unknown').strip().lower() or 'unknown'

        for flag in (lpe.get('contradiction_flags') or []):
            contradiction_flags.append(str(flag))

        chain_data = lpe.get('chain', {})
        ...
        chain_response_action_id = chain_data.get('response_action_id') or None
        ...
        if ev_source in ('live', 'live_provider'):
            ...
            evidence_source = 'unknown'
            blockers.append('evidence source unknown in launch-proof; failing closed')
    ...
    return {
        ...
        'provider_ready': evm_rpc_configured and provider_health_checked,
        ...
        'chain': {
            ...
            'response_action_id': chain_response_action_id,
...
def generate_staging_proof(
    ...
    if release_proof_dir is None:
        release_proof_dir = REPO_ROOT / 'artifacts' / 'release-proof' / 'latest'
    if live_evidence_proof_dir is None:
