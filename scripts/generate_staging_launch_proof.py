...
def build_live_provider_validation(
    ...
    live_evidence_proof = _load_json_artifact(live_evidence_proof_path)
    ...
    # Only use the live-evidence-proof when it has confirmed live_evidence_ready=true.
    # When it's fail-closed (live_evidence_ready=false), fall through to launch-proof
    # for backward compatibility with existing proof workflows.
    _live_ev_proof_active = (
        live_evidence_proof is not None
        and bool(
            live_evidence_proof.get('live_provider_evidence', {}).get('live_evidence_ready')
        )
    )
