...
def print_fail_closed_remediation() -> None:
    ...
    print('')
    print('Set these in GitHub Actions secrets / Railway / local shell, then run:')
    print('')
    print('  make run-staging-live-proof')
    print('')
    print('Local shell examples:')
    print('  # bash / zsh')
    ...
    print('  $env:STAGING_EVM_RPC_URL="https://mainnet.infura.io/v3/<project-id>"')
    print('  $env:STAGING_EVM_CHAIN_ID="1"')
    print('  $env:STAGING_WORKER_ENABLED="true"')
    print('  make run-staging-live-proof')
    print('')
    print('Blocker 3 (live provider evidence) cannot be proven without a real')
    print('EVM JSON-RPC provider. This local run correctly fails closed; the')
    print('fail-closed guardrail and mocked positive-path tests still pass.')
