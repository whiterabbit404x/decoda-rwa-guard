...
def generate_ci_required_gates(*, mode: str, strict: bool = False) -> dict[str, Any]:
    ...
    # Check paid launch readiness — prefer the launch-proof artifact generated by
    # run_paid_saas_launch_proof.py earlier in the same workflow run (billing
    # credentials already validated there). Fall back to env-var check if absent.
    _lp_path = REPO_ROOT / 'artifacts' / 'launch-proof' / 'latest' / 'summary.json'
    _lp_artifact: dict[str, Any] = {}
    if _lp_path.exists():
        try:
            with open(_lp_path) as _f:
                _lp_data = json.load(_f)
            _lp_readiness = _lp_data.get('readiness', {})
            if (
                bool(_lp_data.get('paid_launch_ready'))
                and bool(_lp_readiness.get('billing_ready'))
                and bool(_lp_readiness.get('billing_webhook_ready'))
                and bool(_lp_readiness.get('email_ready'))
                and bool(_lp_readiness.get('provider_ready'))
            ):
                _lp_artifact = {
            ...
            pass
    paid_launch = _lp_artifact if _lp_artifact else build_paid_launch_readiness()
...
def generate_release_proof(*, mode: str, strict: bool = False) -> dict[str, Any]:
    ...
    launch_proof_data: dict[str, Any] | None = None
    ...
    # Check if launch-proof artifact exists and is passing (pilot_ready OR paid_launch_ready)
    ...
    if launch_proof_path.exists():
        try:
            with open(launch_proof_path) as f:
                launch_proof_data = json.load(f)
            launch_proof_ready = bool(
                launch_proof_data.get('pilot_ready', False)
                or launch_proof_data.get('paid_launch_ready', False)
            )
        except:
            pass
    ...
    return {
        ...
        'paid_launch_ready': bool(launch_proof_data.get('paid_launch_ready', False)) if launch_proof_data else False,
        'blockers': sorted(set(blockers)),
        'warnings': [],
        'evidence_files': [
...
def main(mode: str = 'local', strict: bool = False) -> int:
    ...
    # Do not overwrite an existing launch-proof: run_paid_saas_launch_proof.py (or
    # run_no_billing_launch_proof.py) runs first in the workflow and generates the
    # authoritative launch-proof with real billing credentials. Only generate a
    # fallback launch-proof when none exists yet.
    ...
    if not launch_proof_path.exists():
        ...
        print(f'[generate-release-proof] launch-proof already present — preserving '
              'run_paid_saas_launch_proof.py output (not overwriting)')
