...
```bash
# Generate staging launch proof (requires real credentials)
...
# Run full 100% readiness check in staging mode
...
Either:
- `EVM_RPC_URL` is missing or a placeholder.
- The launch-proof artifact (`artifacts/launch-proof/latest/summary.json`) does not
  show `live_evidence_ready=true`.
- The evidence source in the launch proof is `simulator` or `unknown`.

To fix: Configure a real EVM RPC endpoint and re-run `make generate-release-proof`.
