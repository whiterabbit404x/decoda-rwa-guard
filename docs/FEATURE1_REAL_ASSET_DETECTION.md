...
- Proof harness clears `services/api/artifacts/live_evidence/latest/` before every run and now fails hard when artifact truth checks fail (`worker_monitoring_executed`, `lifecycle_checks_executed`, `anomalies_observed`, non-empty `alerts/runs/incidents/evidence/report`, and tx/event-linked evidence).
- `summary.json` integrity markers must come from runtime execution: `generated_at`, `proof_command`, `target_id`, `asset_id`, `monitoring_worker_name`, `monitoring_run_ids`, `anomalous_tx_hashes`, and `anomaly_kind`.
